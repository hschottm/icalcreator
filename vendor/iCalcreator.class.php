...
$pos         = utf8_strpos( $langstr, ';' );
if ($pos   !== false) {
  $langstr   = utf8_substr( $langstr, 0, $pos );
  $pos       = utf8_strpos( $langstr, ',' );
  if ($pos !== false) {
    $pos     = utf8_strpos( $langstr, ',' );
    $langstr = utf8_substr( $langstr, 0, $pos );
 ...
 */
  function _makeProdid() {
    $this->prodid  = '-//'.$this->unique_id.'//NONSGML kigkonsult.se '.ICALCREATOR_VERSION.'//'.utf8_strtoupper( $this->language );
  }
/**
 ...
 */
  function setXprop( $label, $value, $params=FALSE ) {
    if( empty( $label ))
      return FALSE;
    if( 'X-' != utf8_strtoupper( utf8_substr( $label, 0, 2 )))
      return FALSE;
    ...
    $this->xprop[utf8_strtoupper( $label )] = $xprop;
    return TRUE;
  }
/*********************************************************************************/
/**
 ...
 */
  function deleteProperty( $propName=FALSE, $propix=FALSE ) {
    $propName = ( $propName ) ? utf8_strtoupper( $propName ) : 'X-PROP';
    if( !$propix )
      ...
      case 'URL':
        ...
        foreach ( $this->components as $cix => $component) {
          if( !in_array( $component->objName, array('vevent', 'vtodo', 'vjournal', 'vfreebusy' )))
            continue;
          if( in_array( utf8_strtoupper( $propName ), $mProps )) {
            $component->_getProperties( $propName, $output );
            continue;
          }
          elseif(( 3 < utf8_strlen( $propName )) && ( 'UID' == utf8_substr( $propName, -3 ))) {
 ...
 */
  function setProperty () {
    ...
    $arglist[0] = utf8_strtoupper( $arglist[0] );
    switch( $arglist[0] ) {
      case 'CALSCALE':
 ...
 */
  function setConfig( $config, $value = FALSE) {
    if( is_array( $config )) {
      ...
      foreach( $ak as $k ) {
        if( 'DIRECTORY' == utf8_strtoupper( $k )) {
          if( FALSE === $this->setConfig( 'DIRECTORY', $config[$k] ))
            return FALSE;
          unset( $config[$k] );
        }
        elseif( 'NEWLINECHAR' == utf8_strtoupper( $k )) {
    ...
    switch( utf8_strtoupper( $config )) {
      ...
      case 'DIRECTORY':
        ...
        if( $del == utf8_substr( $value, ( 0 - utf8_strlen( $del ))))
          $value = utf8_substr( $value, 0, ( utf8_strlen( $value ) - utf8_strlen( $del )));
        if( is_dir( $value )) {
            /* local directory */
          clearstatcache();
      ...
      case 'FILENAME':
        ...
        break;
      case 'FORMAT':
        $value   = trim( utf8_strtolower( $value ));
        if( 'xcal' == $value ) {
          $this->format             = 'xcal';
 ...
 */
  function deleteComponent( $arg1, $arg2=FALSE  ) {
    ...
    elseif(( utf8_strlen( $arg1 ) <= utf8_strlen( 'vfreebusy' )) && ( FALSE === utf8_strpos( $arg1, '@' ))) {
      $argType = utf8_strtolower( $arg1 );
 ...
 */
  function getComponent( $arg1=FALSE, $arg2=FALSE ) {
    ...
    elseif(( utf8_strlen( $arg1 ) <= utf8_strlen( 'vfreebusy' )) && ( FALSE === utf8_strpos( $arg1, '@' ))) { // object class name
    ...
    elseif(( utf8_strlen( $arg1 ) > utf8_strlen( 'vfreebusy' )) && ( FALSE !== utf8_strpos( $arg1, '@' ))) { // UID as 1st argument
      if( !$arg2 )
      ...
      elseif( is_array( $arg1 )) { // array( *[propertyName => propertyValue] )
        ...
        foreach( $arg1 as $pName => $pValue ) {
          ...
          foreach( $value as $part ) {
            $part = ( FALSE !== utf8_strpos( $part, ',' )) ? explode( ',', $part ) : array( $part );
 ...
 */
  function & newComponent( $compType ) {
    ...
    $ix     = end( $keys) + 1;
    switch( utf8_strtoupper( $compType )) {
      case 'EVENT':
      case 'VEVENT':
...
//echo 'selectComp arg='.date( 'Y-m-d H:i:s', $startDate).' -- '.date( 'Y-m-d H:i:s', $endDate)."<br />\n"; $tcnt = 0;// test ###
    ...
    if( is_array( $cType )) {
      foreach( $cType as $cix => $theType ) {
        $cType[$cix] = $theType = utf8_strtolower( $theType );
  ...
  function selectComponents2( $selectOptions ) {
    ...
    foreach( $this->components as $cix => $component3 ) {
      ...
      foreach( $selectOptions as $propName => $pvalue ) {
        $propName = utf8_strtoupper( $propName );
 ...
 */
  function setComponent( $component, $arg1=FALSE, $arg2=FALSE  ) {
    ...
    elseif( in_array( utf8_strtolower( $arg1 ), array( 'vevent', 'vtodo', 'vjournal', 'vfreebusy', 'valarm', 'vtimezone' ))) {
      $argType = utf8_strtolower( $arg1 );
 ...
 */
  function sort( $sortArg=FALSE ) {
    if( is_array( $this->components )) {
      if( $sortArg ) {
        $sortArg = utf8_strtoupper( $sortArg );
 ...
 */
  function parse( $unparsedtext=FALSE ) {
    ...
    foreach( $rows as $lix => $line ) {
      if(     'BEGIN:VCALENDAR' == utf8_strtoupper( utf8_substr( $line, 0, 15 ))) {
        $calsync++;
        continue;
      }
      elseif( 'END:VCALENDAR'   == utf8_strtoupper( utf8_substr( $line, 0, 13 ))) {
        if( 0 < $compsync )
      ...
      elseif( 1 != $calsync )
        return FALSE;                 /* err 20 */
      elseif( in_array( utf8_strtoupper( utf8_substr( $line, 0, 6 )), $endtxt )) {
        $this->components[] = $comp->copy();
        $compsync--;
        continue;
      }
      if(     'BEGIN:VEVENT'    == utf8_strtoupper( utf8_substr( $line, 0, 12 ))) {
        $comp = new vevent( $config );
        $compsync++;
      }
      elseif( 'BEGIN:VFREEBUSY' == utf8_strtoupper( utf8_substr( $line, 0, 15 ))) {
        $comp = new vfreebusy( $config );
        $compsync++;
      }
      elseif( 'BEGIN:VJOURNAL'  == utf8_strtoupper( utf8_substr( $line, 0, 14 ))) {
        $comp = new vjournal( $config );
        $compsync++;
      }
      elseif( 'BEGIN:VTODO'     == utf8_strtoupper( utf8_substr( $line, 0, 11 ))) {
        $comp = new vtodo( $config );
        $compsync++;
      }
      elseif( 'BEGIN:VTIMEZONE' == utf8_strtoupper( utf8_substr( $line, 0, 15 ))) {
    ...
    if( isset( $this->unparsed ) && is_array( $this->unparsed ) && ( 0 < count( $this->unparsed ))) {
      ...
      foreach( $this->unparsed as $line ) {
        ...
        foreach ( $propnames as $propname ) {
          if( $propname == utf8_strtolower( utf8_substr( $line, 0, utf8_strlen( $propname )))) {
            $newProp = TRUE;
            break;
          }
        }
      ...
      foreach( $proprows as $line ) {
        ...
        $line = str_replace( '!"#¤%&/()=?', '', $line );
        if( '\n' == utf8_substr( $line, -2 ))
          $line = utf8_substr( $line, 0, -2 );
            /* get property name */
        $propname  = null;
        $cix       = 0;
        while( FALSE !== ( $char = utf8_substr( $line, $cix, 1 ))) {
        ...
        if( in_array( utf8_strtoupper( $propname ), array( 'VERSION', 'PRODID' )))
          continue;
        $line = utf8_substr( $line, $cix);
            /* separate attributes from value */
        $attr         = array();
        $attrix       = -1;
        $strlen       = utf8_strlen( $line );
        $WithinQuotes = FALSE;
        $cix          = 0;
        while( FALSE !== utf8_substr( $line, $cix, 1 )) {
          if(                       ( ':'  == $line[$cix] )                         &&
                                    ( utf8_substr( $line,$cix,     3 )  != '://' )       &&
             ( !in_array( utf8_strtolower( utf8_substr( $line,$cix - 6, 4 )), $paramMStz ))   &&
             ( !in_array( utf8_strtolower( utf8_substr( $line,$cix - 3, 4 )), $paramProto3 )) &&
             ( !in_array( utf8_strtolower( utf8_substr( $line,$cix - 4, 5 )), $paramProto4 )) &&
                        ( utf8_strtolower( utf8_substr( $line,$cix - 6, 7 )) != 'mailto:' )   &&
               !$WithinQuotes ) {
            $attrEnd = TRUE;
            if(( $cix < ( $strlen - 4 )) &&
                 ctype_digit( utf8_substr( $line, $cix+1, 4 ))) { // an URI with a (4pos) portnr??
              for( $c2ix = $cix; 3 < $c2ix; $c2ix-- ) {
                if( '://' == utf8_substr( $line, $c2ix - 2, 3 )) {
                  $attrEnd = FALSE;
        ...
        foreach( $attr as $attribute ) {
          ...
          if( 1 < count( $attrsplit ))
            ...
            /* update Property */
        if( FALSE !== utf8_strpos( $line, ',' )) {
          $content  = array( 0 => '' );
          $cix = $lix = 0;
          while( FALSE !== utf8_substr( $line, $lix, 1 )) {
 ...
 */
  function calendarComponent() {
    $this->objName         = ( isset( $this->timezonetype )) ?
                          utf8_strtolower( $this->timezonetype )  :  get_class ( $this );
    $this->uid             = array();
    $this->dtstamp         = array();
    ...
    foreach( $this->attach as $attachPart ) {
      if( !empty( $attachPart['value'] )) {
        ...
        if(( 'xcal' != $this->format ) && isset( $attachPart['params']['VALUE'] ) && ( 'BINARY' == $attachPart['params']['VALUE'] )) {
          ...
          $output     = utf8_substr( $str, 0, 75 ).$this->nl;
          $str        = utf8_substr( $str, 75 );
          $output    .= ' '.chunk_split( $str, 74, $this->nl.' ' );
          if( ' ' == utf8_substr( $output, -1 ))
            $output   = rtrim( $output );
          if( $this->nl != utf8_substr( $output, ( 0 - utf8_strlen( $this->nl ))))
            $output  .= $this->nl;
          return $output;
        }
      ...
      foreach( $attendeePart as $paramlabel => $paramvalue ) {         // start foreach 2
        ...
        elseif(( 'params' == $paramlabel ) && ( is_array( $paramvalue ))) { // start elseif
          ...
          foreach( $paramvalue as $pKey => $pValue ) {                 // fix (opt) quotes
            if( is_array( $pValue ) || in_array( $pKey, $mParams ))
              continue;
            if(( FALSE !== utf8_strpos( $pValue, ':' )) ||
               ( FALSE !== utf8_strpos( $pValue, ';' )) ||
               ( FALSE !== utf8_strpos( $pValue, ',' )))
        ...
        // set attenddee parameters in rfc2445 order
          ...
          if( isset( $paramvalue['DIR'] )) {
            $delim = ( FALSE === utf8_strpos( $paramvalue['DIR'], '"' )) ? '"' : '';
 ...
 */
  function setAttendee( $value, $params=FALSE, $index=FALSE ) {
    ...
    if( !empty( $value )) {
      if( FALSE === ( $pos = utf8_strpos( utf8_substr( $value, 0, 9 ), ':' )))
        $value = 'MAILTO:'.$value;
      elseif( !empty( $value ))
        $value = utf8_strtolower( utf8_substr( $value, 0, $pos )).utf8_substr( $value, $pos );
    ...
    if( is_array($params )) {
      ...
      foreach( $params as $optparamlabel => $optparamvalue ) {
        $optparamlabel = utf8_strtoupper( $optparamlabel );
        switch( $optparamlabel ) {
          ...
          case 'DELEGATED-FROM':
            ...
            foreach( $optparamvalue as $part ) {
              $part = trim( $part );
              if(( '"' == utf8_substr( $part, 0, 1 )) &&
                 ( '"' == utf8_substr( $part, -1 )))
                $part = utf8_substr( $part, 1, ( utf8_strlen( $part ) - 2 ));
              if( 'mailto:' != utf8_strtolower( utf8_substr( $part, 0, 7 )))
                $part = "MAILTO:$part";
              else
                $part = 'MAILTO:'.utf8_substr( $part, 7 );
              $optarrays[$optparamlabel][] = $part;
            }
            break;
          default:
            if(( '"' == utf8_substr( $optparamvalue, 0, 1 )) &&
               ( '"' == utf8_substr( $optparamvalue, -1 )))
              $optparamvalue = utf8_substr( $optparamvalue, 1, ( utf8_strlen( $optparamvalue ) - 2 ));
            if( 'SENT-BY' ==  $optparamlabel ) {
              if( 'mailto:' != utf8_strtolower( utf8_substr( $optparamvalue, 0, 7 )))
                $optparamvalue = "MAILTO:$optparamvalue";
              else
                $optparamvalue = 'MAILTO:'.utf8_substr( $optparamvalue, 7 );
            }
 ...
 */
  function setDuration( $week, $day=FALSE, $hour=FALSE, $min=FALSE, $sec=FALSE, $params=FALSE ) {
    ...
    elseif( is_string( $week ) && ( 3 <= utf8_strlen( trim( $week )))) {
      $week = trim( $week );
      if( in_array( utf8_substr( $week, 0, 1 ), array( '+', '-' )))
        $week = utf8_substr( $week, 1 );
      ...
      foreach( $theExdate['value'] as $eix => $exdatePart ) {
        ...
        if( 0 < $eix ) {
          if( isset( $theExdate['value'][0]['tz'] )) {
            if( ctype_digit( utf8_substr( $theExdate['value'][0]['tz'], -4 )) ||
               ( 'Z' == $theExdate['value'][0]['tz'] )) {
              if( 'Z' != utf8_substr( $formatted, -1 ))
                $formatted .= 'Z';
            }
            else
              $formatted = str_replace( 'Z', '', $formatted );
 ...
 */
  function setExdate( $exdates, $params=FALSE, $index=FALSE ) {
    ...
    $toZ = ( isset( $input['params']['TZID'] ) && in_array( utf8_strtoupper( $input['params']['TZID'] ), array( 'GMT', 'UTC', 'Z' ))) ? TRUE : FALSE;
    ...
    foreach( $exdates as $eix => $theExdate ) {
      ...
      elseif( 8 <= utf8_strlen( trim( $theExdate ))) { // ex. 2006-08-03 10:12:18
  ...
  function setFreebusy( $fbType, $fbValues, $params=FALSE, $index=FALSE ) {
    ...
    foreach( $fbValues as $fbPeriod ) {   // periods => period
      ...
      foreach( $fbPeriod as $fbMember ) { // pairs => singlepart
        ...
        elseif(( 3 <= utf8_strlen( trim( $fbMember ))) &&    // string format duration
               ( in_array( $fbMember{0}, array( 'P', '+', '-' )))) {
          if( 'P' != $fbMember{0} )
            $fbmember = utf8_substr( $fbMember, 1 );
          $freebusyPairMember = iCalUtilityFunctions::_duration_string( $fbMember );
        }
        elseif( 8 <= utf8_strlen( trim( $fbMember ))) { // text date ex. 2006-08-03 10:12:18
 ...
 */
  function setOrganizer( $value, $params=FALSE ) {
    ...
    if( isset( $this->organizer['params']['SENT-BY'] )){
      if( 'mailto:' !== utf8_strtolower( utf8_substr( $this->organizer['params']['SENT-BY'], 0, 7 )))
        $this->organizer['params']['SENT-BY'] = 'MAILTO:'.$this->organizer['params']['SENT-BY'];
      else
        $this->organizer['params']['SENT-BY'] = 'MAILTO:'.utf8_substr( $this->organizer['params']['SENT-BY'], 7 );
    }
    return TRUE;
  }
/*********************************************************************************/
        ...
        if( is_array( $rdatePart ) &&
          ...
          if( 0 < $rpix ) {
            if( !empty( $rdatePart[0]['tz'] ) && iCalUtilityFunctions::_isOffset( $rdatePart[0]['tz'] )) {
              if( 'Z' != utf8_substr( $formatted, -1 )) $formatted .= 'Z';
            }
            else
              $formatted = str_replace( 'Z', '', $formatted );
          }
 ...
 */
  function setRdate( $rdates, $params=FALSE, $index=FALSE ) {
    ...
    $toZ = ( isset( $params['TZID'] ) && in_array( utf8_strtoupper( $params['TZID'] ), $zArr )) ? TRUE : FALSE;
    ...
    (( is_array( $rdates[0][0] ) && ( isset( $rdates[0][0]['timestamp'] ) ||
                                    ...
                                    ( is_string( $rdates[0][0] ) && ( 8 <= utf8_strlen( trim( $rdates[0][0] )))))  &&
     ( is_array( $rdates[0][1] ) || ( is_string( $rdates[0][1] ) && ( 3 <= utf8_strlen( trim( $rdates[0][1] ))))))
    ...
    foreach( $rdates as $rpix => $theRdate ) {
      ...
      if( is_array( $theRdate )) {
        if( isset( $input['params']['VALUE'] ) && ( 'PERIOD' == $input['params']['VALUE'] )) { // PERIOD
          foreach( $theRdate as $rix => $rPeriod ) {
            ...
            if( is_array( $rPeriod )) {
              ...
              elseif (( 1 == count( $rPeriod )) && ( 8 <= utf8_strlen( reset( $rPeriod )))) { // text-date
            ...
            elseif(( 3 <= utf8_strlen( trim( $rPeriod ))) &&          // string format duration
                   ( in_array( $rPeriod[0], array( 'P', '+', '-' )))) {
              if( 'P' != $rPeriod[0] )
                $rPeriod  = utf8_substr( $rPeriod, 1 );
              $inputab    = iCalUtilityFunctions::_duration_string( $rPeriod );
            }
            elseif( 8 <= utf8_strlen( trim( $rPeriod ))) {            // text date ex. 2006-08-03 10:12:18
        ...
        elseif ( iCalUtilityFunctions::_isArrayTimestampDate( $theRdate )) {    // timestamp
          ...
          $toZ = ( isset( $inputa['tz'] ) && in_array( utf8_strtoupper( $inputa['tz'] ), $zArr )) ? TRUE : FALSE;
          if( $toZ )
            $inputa['tz']   = 'Z';
        }
      }
      elseif( 8 <= utf8_strlen( trim( $theRdate ))) {                 // text date ex. 2006-08-03 10:12:18
  ...
  function setTrigger( $year, $month=null, $day=null, $week=FALSE, $hour=FALSE, $min=FALSE, $sec=FALSE, $relatedStart=TRUE, $before=TRUE, $params=FALSE ) {
    ...
    elseif( is_array( $year ) && ( is_array( $month ) || empty( $month ))) {
      ...
      if(!(array_key_exists( 'year',  $year ) &&   // exclude date-time
        ...
        if( isset( $params['RELATED'] ) && ( 'END' == utf8_strtoupper( $params['RELATED'] )))
          $relatedStart = FALSE;
        else
    ...
    elseif(is_string( $year ) && ( is_array( $month ) || empty( $month ))) {  // duration or date in a string
      ...
      if( in_array( $year[0], array( 'P', '+', '-' ))) { // duration
        $relatedStart = ( isset( $params['RELATED'] ) && ( 'END' == utf8_strtoupper( $params['RELATED'] ))) ? FALSE : TRUE;
        $before       = ( '-'  == $year[0] ) ? TRUE : FALSE;
        if(     'P'  != $year[0] )
          $year       = utf8_substr( $year, 1 );
 ...
 */
  function _makeUid() {
    $date   = date('Ymd\THisT');
    $unique = utf8_substr(microtime(), 2, 4);
    $base   = 'aAbBcCdDeEfFgGhHiIjJkKlLmMnNoOpPrRsStTuUvVxXuUvVwWzZ1234567890';
    $start  = 0;
    $end    = utf8_strlen( $base ) - 1;
    $length = 6;
    $str    = null;
    for( $p = 0; $p < $length; $p++ )
    ...
    switch( $this->format ) {
      case 'xcal':
        $objectname               = ( isset( $this->timezonetype )) ?
                                 utf8_strtolower( $this->timezonetype )  :  utf8_strtolower( $this->objName );
      ...
      default:
        $objectname               = ( isset( $this->timezonetype )) ?
                                 utf8_strtoupper( $this->timezonetype )  :  utf8_strtoupper( $this->objName );
 ...
 */
  function _createElement( $label, $attributes=null, $content=FALSE ) {
    ...
    $attachfmttype      = null;
    if (( 'xcal' == $this->format) && ( 'x-' == utf8_substr( $label, 0, 2 ))) {
      ...
      if ( 'xcal' == $this->format ) {
        ...
        foreach( $attributes2 as $aix => $attribute ) {
          ...
          elseif( 2 == count( $attrKVarr)) {
            $attrKey   = utf8_strtolower( $attrKVarr[0] );
          ...
          elseif(( 'categories' == $label ) && ( 'language' == $attrKey ))
            ...
            if(( '"' == utf8_substr( $attrValue, 0, 1 )) && ( '"' == utf8_substr( $attrValue, -1 ))) {
              $attrValue = utf8_substr( $attrValue, 1, ( utf8_strlen( $attrValue ) - 2 ));
    ...
    if(( 'xcal' == $this->format) &&
      ...
      $pos = strrpos($content, "/");
      $docname = ( $pos !== false) ? utf8_substr( $content, (1 - utf8_strlen( $content ) + $pos )) : $content;
 ...
 */
  function _createParams( $params=array(), $ctrKeys=array() ) {
    ...
    foreach( $params as $paramKey => $paramValue ) {
      if(( FALSE !== utf8_strpos( $paramValue, ':' )) ||
         ( FALSE !== utf8_strpos( $paramValue, ';' )) ||
         ( FALSE !== utf8_strpos( $paramValue, ',' )))
      ...
      $paramKey             = utf8_strtoupper( $paramKey );
    ...
    if( isset( $params['DIR'] )      && in_array( 'DIR',      $ctrKeys )) {
      $delim = ( FALSE !== utf8_strpos( $params['DIR'], '"' )) ? '' : '"';
    ...
    if( isset( $params['ALTREP'] )   && in_array( 'ALTREP',   $ctrKeys )) {
      $delim = ( FALSE !== utf8_strpos( $params['ALTREP'], '"' )) ? '' : '"';
 ...
 */
  function _notExistProp( $propName ) {
    ...
    $propName = utf8_strtolower( $propName );
    ...
    elseif((       'x-' != utf8_substr($propName,0,2)) && !isset( $this->$propName ))       return TRUE;
    return FALSE;
  }
/*********************************************************************************/
/*********************************************************************************/
 ...
 */
  function setConfig( $config, $value = FALSE, $softUpdate = FALSE ) {
    if( is_array( $config )) {
      $ak = array_keys( $config );
      foreach( $ak as $k ) {
        if( 'NEWLINECHAR' == utf8_strtoupper( $k )) {
          if( FALSE === $this->setConfig( 'NEWLINECHAR', $config[$k] ))
            return FALSE;
          unset( $config[$k] );
          break;
 ...
 */
  function getProperty( $propName=FALSE, $propix=FALSE, $inclParam=FALSE, $specform=FALSE ) {
    if( 'GEOLOCATION' == utf8_strtoupper( $propName )) {
 ...
 */
  function _getProperties( $propName, & $output ) {
    if( empty( $output ))
      $output = array();
    if( !in_array( utf8_strtoupper( $propName ), array( 'ATTENDEE', 'CATEGORIES', 'CONTACT', 'RELATED-TO', 'RESOURCES' )))
      return $output;
    while( FALSE !== ( $content = $this->getProperty( $propName ))) {
      ...
      elseif( FALSE !== utf8_strpos( $content, ',' )) {
      ...
      } // end elseif( FALSE !== utf8_strpos( $content, ',' ))
      else {
 ...
 */
  function parse( $unparsedtext=null ) {
    ...
    $compsync = $subsync = 0;
    foreach ( $unparsedtext as $lix => $line ) {
      if( 'END:VALARM'         == utf8_strtoupper( utf8_substr( $line, 0, 10 ))) {
        if( 1 != $subsync ) return FALSE;
        $this->components[]     = $comp->copy();
        $subsync--;
      }
      elseif( 'END:DAYLIGHT'   == utf8_strtoupper( utf8_substr( $line, 0, 12 ))) {
        if( 1 != $subsync ) return FALSE;
        $this->components[]     = $comp->copy();
        $subsync--;
      }
      elseif( 'END:STANDARD'   == utf8_strtoupper( utf8_substr( $line, 0, 12 ))) {
        if( 1 != $subsync ) return FALSE;
        array_unshift( $this->components, $comp->copy());
        $subsync--;
      }
      elseif( 'END:'           == utf8_strtoupper( utf8_substr( $line, 0, 4 ))) { // end:<component>
      ...
      elseif( 'BEGIN:VALARM'   == utf8_strtoupper( utf8_substr( $line, 0, 12 ))) {
        $comp = new valarm( $config);
        $subsync++;
      }
      elseif( 'BEGIN:STANDARD' == utf8_strtoupper( utf8_substr( $line, 0, 14 ))) {
        $comp = new vtimezone( 'standard', $config );
        $subsync++;
      }
      elseif( 'BEGIN:DAYLIGHT' == utf8_strtoupper( utf8_substr( $line, 0, 14 ))) {
        $comp = new vtimezone( 'daylight', $config );
        $subsync++;
      }
      elseif( 'BEGIN:'         == utf8_strtoupper( utf8_substr( $line, 0, 6 )))  // begin:<component>
        $compsync++;
      else
        $comp->unparsed[]       = $line;
    }
    ...
    foreach( $proprows as $line ) {
      ...
      if(( 'x-' == utf8_substr( $propname, 0, 2 )) || ( 'X-' == utf8_substr( $propname, 0, 2 ))) {
        $propname2 = $propname;
        $propname  = 'X-';
      }
      ...
      switch( utf8_strtoupper( $propname )) {
        case 'ATTENDEE':
          foreach( $propattr as $pix => $attr ) {
            if( !in_array( utf8_strtoupper( $pix ), array( 'MEMBER', 'DELEGATED-TO', 'DELEGATED-FROM' )))
              continue;
            $attr2 = explode( ',', $attr );
              if( 1 < count( $attr2 ))
                $propattr[$pix] = $attr2;
        ...
        case 'RRULE':
          ...
          foreach( $values as $value2 ) {
            ...
            $rulelabel = utf8_strtoupper( $value3[0] );
            switch( $rulelabel ) {
              case 'BYDAY': {
                ...
                if( 1 < count( $value4 )) {
                  foreach( $value4 as $v5ix => $value5 ) {
                    ...
                    if(( ctype_alpha( utf8_substr( $value5, -1 ))) &&
                       ( ctype_alpha( utf8_substr( $value5, -2, 1 )))) {
                      $dayname = utf8_substr( $value5, -2, 2 );
                      if( 2 < utf8_strlen( $value5 ))
                        $dayno = utf8_substr( $value5, 0, ( utf8_strlen( $value5 ) - 2 ));
                    }
                    if( $dayno )
                      $value6[] = $dayno;
                    if( $dayname )
    ...
    switch( $this->format ) {
      ...
      default:
        ...
        while( isset( $string[$pos] )) {
          if( FALSE === ( $pos = utf8_strpos( $string, "\\", $pos )))
            break;
          if( !in_array( utf8_substr( $string, $pos, 1 ), $specChars )) {
            $string = utf8_substr( $string, 0, $pos )."\\".utf8_substr( $string, ( $pos + 1 ));
            $pos += 1;
          }
          $pos += 1;
        }
        if( FALSE !== utf8_strpos( $string, '"' ))
        ...
        if( FALSE !== utf8_strpos( $string, "\r\n" ))
          $string = str_replace( "\r\n", '\n',    $string);
        elseif( FALSE !== utf8_strpos( $string, "\r" ))
          $string = str_replace( "\r", '\n',      $string);
        elseif( FALSE !== utf8_strpos( $string, "\n" ))
          $string = str_replace( "\n", '\n',      $string);
        if( FALSE !== utf8_strpos( $string, '\N' ))
          $string = str_replace( '\N', '\n',      $string);
//        if( FALSE !== utf8_strpos( $string, $this->nl ))
 ...
 */
  function vtimezone( $timezonetype=FALSE, $config = array()) {
    ...
    if( !$timezonetype )
      ...
      $this->timezonetype = utf8_strtoupper( $timezonetype );
    $this->calendarComponent();
  ...
  public static function _chkdatecfg( $theDate, & $parno, & $params ) {
    ...
    elseif( isset( $params['VALUE'] ) && ( 'DATE' == $params['VALUE'] ))
      ...
      if( is_array( $theDate )) {
        ...
        $date = trim( $theDate );
        if( 'Z' == utf8_substr( $date, -1 ))
          $parno = 7; // UTC DATE-TIME
        elseif((( 8 == utf8_strlen( $date ) && ctype_digit( $date )) || ( 11 >= utf8_strlen( $date ))) &&
 ...
 */
  public static function createTimezone( & $calendar, $timezone, $xProp=array(), $from=null, $to=null ) {
    if( utf8_substr( phpversion(), 0, 3 ) < '5.2' )
      return FALSE;
    if( empty( $timezone ))
      return FALSE;
    try {
    ...
    if( !empty( $xProp )) {
      foreach( $xProp as $xPropName => $xPropValue )
        if( 'x-' == utf8_strtolower( utf8_substr( $xPropName, 0, 2 )))
 ...
 */
  public static function _date_time_string( $datetime, $parno=FALSE ) {
    ...
    $tz  = null;
    $len = utf8_strlen( $datetime ) - 1;
    if( 'Z' == utf8_substr( $datetime, -1 )) {
      $tz = 'Z';
      $datetime = trim( utf8_substr( $datetime, 0, $len ));
    }
    elseif( ( ctype_digit( utf8_substr( $datetime, -2, 2 ))) && // time or date
                  ( '-' == utf8_substr( $datetime, -3, 1 )) ||
                  ( ':' == utf8_substr( $datetime, -3, 1 )) ||
                  ( '.' == utf8_substr( $datetime, -3, 1 ))) {
      $continue = TRUE;
    }
    elseif( ( ctype_digit( utf8_substr( $datetime, -4, 4 ))) && // 4 pos offset
            ( ' +' == utf8_substr( $datetime, -6, 2 )) ||
            ( ' -' == utf8_substr( $datetime, -6, 2 ))) {
      $tz = utf8_substr( $datetime, -5, 5 );
      $datetime = utf8_substr( $datetime, 0, ($len - 5));
    }
    elseif( ( ctype_digit( utf8_substr( $datetime, -6, 6 ))) && // 6 pos offset
            ( ' +' == utf8_substr( $datetime, -8, 2 )) ||
            ( ' -' == utf8_substr( $datetime, -8, 2 ))) {
      $tz = utf8_substr( $datetime, -7, 7 );
      $datetime = utf8_substr( $datetime, 0, ($len - 7));
    }
    elseif( ( 6 < $len ) && ( ctype_digit( utf8_substr( $datetime, -6, 6 )))) {
      $continue = TRUE;
    }
    elseif( 'T' ==  utf8_substr( $datetime, -7, 1 )) {
      $continue = TRUE;
    }
    else {
      $cx  = $tx = 0;    //  19970415T133000 US-Eastern
      for( $cx = -1; $cx > ( 9 - $len ); $cx-- ) {
        $char = utf8_substr( $datetime, $cx, 1 );
      ...
      if( 0 > $tx ) {
        $tz = utf8_substr( $datetime, $tx );
        $datetime = trim( utf8_substr( $datetime, 0, $len + $tx + 1 ));
      }
    }
    if( 0 < substr_count( $datetime, '-' )) {
      $datetime = str_replace( '-', '/', $datetime );
    }
    elseif( ctype_digit( utf8_substr( $datetime, 0, 8 )) &&
           ( 'T' ==      utf8_substr( $datetime, 8, 1 )) &&
            ctype_digit( utf8_substr( $datetime, 9, 6 ))) {
     }
    $datestring = date( 'Y-m-d H:i:s', strtotime( $datetime ));
    $tz                = trim( $tz );
    $output            = array();
    $output['year']    = utf8_substr( $datestring, 0, 4 );
    $output['month']   = utf8_substr( $datestring, 5, 2 );
    $output['day']     = utf8_substr( $datestring, 8, 2 );
    if(( 6 == $parno ) || ( 7 == $parno ) || ( !$parno && ( 'Z' == $tz ))) {
      $output['hour']  = utf8_substr( $datestring, 11, 2 );
      $output['min']   = utf8_substr( $datestring, 14, 2 );
      $output['sec']   = utf8_substr( $datestring, 17, 2 );
      if( !empty( $tz ))
        $output['tz']  = $tz;
    }
    elseif( 3 != $parno ) {
      if(( '00' < utf8_substr( $datestring, 11, 2 )) ||
         ( '00' < utf8_substr( $datestring, 14, 2 )) ||
         ( '00' < utf8_substr( $datestring, 17, 2 ))) {
        $output['hour']  = utf8_substr( $datestring, 11, 2 );
        $output['min']   = utf8_substr( $datestring, 14, 2 );
        $output['sec']   = utf8_substr( $datestring, 17, 2 );
      }
 ...
 */
  public static function _duration_string( $duration ) {
    ...
    while( 'P' != utf8_strtoupper( utf8_substr( $duration, 0, 1 ))) {
      if( 0 < utf8_strlen( $duration ))
        $duration = utf8_substr( $duration, 1 );
      else
        return false; // no leading P !?!?
    }
    $duration = utf8_substr( $duration, 1 ); // skip P
    $duration = str_replace ( 't', 'T', $duration );
    $duration = str_replace ( 'T', '', $duration );
    $output = array();
    $val    = null;
    for( $ix=0; $ix < utf8_strlen( $duration ); $ix++ ) {
      switch( utf8_strtoupper( utf8_substr( $duration, $ix, 1 ))) {
       case 'W':
         $output['week'] = $val;
         $val            = null;
         break;
       ...
       case 'S':
         ...
         break;
       default:
         if( !ctype_digit( utf8_substr( $duration, $ix, 1 )))
           return false; // unknown duration control character  !?!?
         else
           $val .= utf8_substr( $duration, $ix, 1 );
      }
    }
 ...
 */
  public static function _existRem( &$array, $expkey, $expval=FALSE, $hitVal=null, $elseVal=null, $preSet=null ) {
    ...
    foreach( $array as $key => $value ) {
      if( utf8_strtoupper( $expkey ) == utf8_strtoupper( $key )) {
        if( !$expval || ( utf8_strtoupper( $expval ) == utf8_strtoupper( $array[$key] ))) {
          unset( $array[$key] );
          return $hitVal;
        }
      }
 ...
 */
  public static function _isOffset( $input ) {
    ...
    if( 'Z' == utf8_substr( $input, -1 ))
      return TRUE;
    elseif((   5 <= utf8_strlen( $input )) &&
       ( in_array( utf8_substr( $input, -5, 1 ), array( '+', '-' ))) &&
       (   '0000'  < utf8_substr( $input, -4 )) && (   '9999' >= utf8_substr( $input, -4 )))
      return TRUE;
    elseif((    7 <= utf8_strlen( $input )) &&
       ( in_array( utf8_substr( $input, -7, 1 ), array( '+', '-' ))) &&
       ( '000000'  < utf8_substr( $input, -6 )) && ( '999999' >= utf8_substr( $input, -6 )))
      return TRUE;
    return FALSE;
  }
/**
 ...
 */
  public static function ms2phpTZ( & $timezone ) {
    ...
    if( '(UTC' != utf8_substr( $search, 0, 4 ))
      return FALSE;
    if( FALSE === ( $pos = utf8_strpos( $search, ')' )))
      return FALSE;
    $pos    = utf8_strpos( $search, ')' );
    $searchOffset = utf8_substr( $search, 4, ( $pos - 4 ));
    $searchOffset = iCalUtilityFunctions::_tz2offset( str_replace( ':', '', $searchOffset ));
    while( ' ' ==utf8_substr( $search, ( $pos + 1 )))
      $pos += 1;
    $searchText   = trim( str_replace( array( '(', ')', '&', ',', '  ' ), ' ', utf8_substr( $search, ( $pos + 1 )) ));
      ...
      foreach( $transitions as $cnt => $transition ) {
        ...
        foreach( $cWords as $cWord ) {
          ...
          foreach( $searchWords as $sWord ) {
            if( empty( $sWord ) || ( 'time' == utf8_strtolower( $sWord )))
              continue;
            $sPrio += 1;
            if( utf8_strtolower( $cWord ) == utf8_strtolower( $sWord )) {
              $hitCnt += 1;
              $rank   += ( $cPrio + $sPrio );
            }
            else
 ...
 */
  public static function offsetSec2His( $seconds ) {
    if( '-' == utf8_substr( $seconds, 0, 1 )) {
      $prefix  = '-';
      $seconds = utf8_substr( $seconds, 1 );
    }
    elseif( '+' == utf8_substr( $seconds, 0, 1 )) {
      $prefix  = '+';
      $seconds = utf8_substr( $seconds, 1 );
    }
    else
 ...
 */
  public static function _setRexrule( $rexrule ) {
    ...
    foreach( $rexrule as $rexrulelabel => $rexrulevalue ) {
      $rexrulelabel = utf8_strtoupper( $rexrulelabel );
      if( 'UNTIL'  != $rexrulelabel )
        ...
        elseif( 8 <= utf8_strlen( trim( $rexrulevalue ))) { // ex. textual datetime/date 2006-08-03 10:12:18
    ...
    if( isset( $input['BYDAY'] )) {
      if( !is_array( $input['BYDAY'] )) // ensure upper case.. .
        $input2['BYDAY']    = utf8_strtoupper( $input['BYDAY'] );
      else {
        foreach( $input['BYDAY'] as $BYDAYx => $BYDAYv ) {
          if( 'DAY'        == utf8_strtoupper( $BYDAYx ))
             $input2['BYDAY']['DAY'] = utf8_strtoupper( $BYDAYv );
          elseif( !is_array( $BYDAYv )) {
             $input2['BYDAY'][$BYDAYx]  = $BYDAYv;
          }
          else {
            foreach( $BYDAYv as $BYDAYx2 => $BYDAYv2 ) {
              if( 'DAY'    == utf8_strtoupper( $BYDAYx2 ))
                 $input2['BYDAY'][$BYDAYx]['DAY'] = utf8_strtoupper( $BYDAYv2 );
              else
 ...
 */
  public static function _setDate( $year, $month=FALSE, $day=FALSE, $hour=FALSE, $min=FALSE, $sec=FALSE, $tz=FALSE, $params=FALSE, $caller=null, $objName=null, $tzid=FALSE ) {
    ...
    elseif( 8 <= utf8_strlen( trim( $year ))) { // ex. 2006-08-03 10:12:18
      if( $localtime ) unset ( $month['VALUE'], $month['TZID'] );
      $input['params'] = iCalUtilityFunctions::_setParams( $month, array( 'VALUE' => 'DATE-TIME' ));
      if( isset( $input['params']['TZID'] )) {
        $input['params']['VALUE'] = 'DATE-TIME';
      ...
      elseif( $tzid && iCalUtilityFunctions::_isOffset( utf8_substr( $year, -7 ))) {
        if(( in_array( utf8_substr( $year, -5, 1 ), array( '+', '-' ))) &&
           (   '0000'  < utf8_substr( $year, -4 )) && (   '9999' >= utf8_substr( $year, -4 )))
          $year = utf8_substr( $year, 0, ( utf8_strlen( $year ) - 5 ));
        elseif(( in_array( utf8_substr( $input, -7, 1 ), array( '+', '-' ))) &&
               ( '000000'  < utf8_substr( $input, -6 )) && ( '999999' >= utf8_substr( $input, -6 )))
          $year = utf8_substr( $year, 0, ( utf8_strlen( $year ) - 7 ));
        $parno = 6;
      }
    ...
    if(  isset( $input['params']['TZID'] ) && !empty( $input['params']['TZID'] )) {
      ...
      elseif( in_array( utf8_strtoupper( $input['params']['TZID'] ), array( 'GMT', 'UTC', 'Z' ))) { // time zone Z
        $input['value']['tz'] = 'Z';
        unset( $input['params']['TZID'] );
      }
    }
    ...
    foreach( $params as $paramKey => $paramValue ) {
      if( is_array( $paramValue )) {
        foreach( $paramValue as $pkey => $pValue ) {
          if(( '"' == utf8_substr( $pValue, 0, 1 )) && ( '"' == utf8_substr( $pValue, -1 )))
            $paramValue[$pkey] = utf8_substr( $pValue, 1, ( utf8_strlen( $pValue ) - 2 ));
        }
      }
      elseif(( '"' == utf8_substr( $paramValue, 0, 1 )) && ( '"' == utf8_substr( $paramValue, -1 )))
        $paramValue = utf8_substr( $paramValue, 1, ( utf8_strlen( $paramValue ) - 2 ));
      if( 'VALUE' == utf8_strtoupper( $paramKey ))
        $input['VALUE']                 = utf8_strtoupper( $paramValue );
      else
        $input[utf8_strtoupper( $paramKey )] = $paramValue;
    }
 ...
 */
  public static function _strDate2arr( & $input ) {
    ...
    if( 2 == substr_count( $work, '/' ))
      $work = str_replace( '/', '', $work );
    if( !ctype_digit( utf8_substr( $work, 0, 8 )))
      return FALSE;
    if( !checkdate( (int) utf8_substr( $work,  4, 2 ), (int) utf8_substr( $work,  6, 2 ), (int) utf8_substr( $work,  0, 4 )))
      return FALSE;
    $temp = array( 'year'  => utf8_substr( $work,  0, 4 )
                 , 'month' => utf8_substr( $work,  4, 2 )
                 , 'day'   => utf8_substr( $work,  6, 2 ));
    if( 8 == utf8_strlen( $work )) {
      $input = $temp;
      return TRUE;
    }
    if(( ' ' == utf8_substr( $work, 8, 1 )) || ( 'T' == utf8_substr( $work, 8, 1 )) || ( 't' == utf8_substr( $work, 8, 1 )))
      $work =  utf8_substr( $work, 9 );
    elseif( ctype_digit( utf8_substr( $work, 8, 1 )))
      $work = utf8_substr( $work, 8 );
    else
     return FALSE;
    if( 2 == substr_count( $work, ':' ))
      $work = str_replace( ':', '', $work );
    if( !ctype_digit( utf8_substr( $work, 0, 4 )))
      return FALSE;
    $temp['hour']  = utf8_substr( $work, 0, 2 );
    $temp['min']   = utf8_substr( $work, 2, 2 );
    if((( 0 > $temp['hour'] ) || ( $temp['hour'] > 23 )) ||
       (( 0 > $temp['min'] )  || ( $temp['min']  > 59 )))
      return FALSE;
    if( ctype_digit( utf8_substr( $work, 4, 2 ))) {
      $temp['sec'] = utf8_substr( $work, 4, 2 );
    ...
    if( $len < utf8_strlen( $work))
      $temp['tz'] = trim( utf8_substr( $work, 6 ));
    $input = $temp;
    return TRUE;
  }
/**
 ...
 */
  public static function transformDateTime( & $date, $tzFrom, $tzTo='UTC', $format = 'Ymd\THis' ) {
    ...
    elseif( iCalUtilityFunctions::_isArrayDate( $date )) {
      ...
      if( 'Z' == utf8_substr( $date, -1 ))
        $date = utf8_substr( $date, 0, ( utf8_strlen( $date ) - 2 ));
 ...
 */
  public static function _tz2offset( $tz ) {
    $tz           = trim( (string) $tz );
    $offset       = 0;
    if(((     5  != utf8_strlen( $tz )) && ( 7  != utf8_strlen( $tz ))) ||
       ((    '+' != utf8_substr( $tz, 0, 1 )) && ( '-' != utf8_substr( $tz, 0, 1 ))) ||
       (( '0000' >= utf8_substr( $tz, 1, 4 )) && ( '9999' < utf8_substr( $tz, 1, 4 ))) ||
           (( 7  == utf8_strlen( $tz )) && ( '00' > utf8_substr( $tz, 5, 2 )) && ( '99' < utf8_substr( $tz, 5, 2 ))))
      return $offset;
    $hours2sec    = (int) utf8_substr( $tz, 1, 2 ) * 3600;
    $min2sec      = (int) utf8_substr( $tz, 3, 2 ) *   60;
    $sec          = ( 7  == utf8_strlen( $tz )) ? (int) utf8_substr( $tz, -2 ) : '00';
    $offset       = $hours2sec + $min2sec + $sec;
    $offset       = ('-' == utf8_substr( $tz, 0, 1 )) ? $offset * -1 : $offset;
    return $offset;
  }
}
/*********************************************************************************/
...
function iCal2vCard( $email, $version='2.1', $directory=FALSE, $ext='vcf' ) {
  if( FALSE === ( $pos = utf8_strpos( $email, '@' )))
    return FALSE;
  if( $directory ) {
    if( DIRECTORY_SEPARATOR != utf8_substr( $directory, ( 0 - utf8_strlen( DIRECTORY_SEPARATOR ))))
  ...
  $name   = $person = utf8_substr( $email, 0, $pos );
  if( ctype_upper( $name ) || ctype_lower( $name ))
    $name = array( $name );
  else {
    if( FALSE !== ( $pos = utf8_strpos( $name, '.' ))) {
      $name = explode( '.', $name );
      ...
      $k     = 0;
      $x     = 1;
      while( FALSE !== ( $char = utf8_substr( $chars, $x, 1 ))) {
        if( ctype_upper( $char )) {
          $k += 1;
          $name[$k] = '';
        }
...
function iCal2vCards( & $calendar, $version='2.1', $directory=FALSE, $ext='vcf' ) {
  ...
  foreach( $vCardP as $prop ) {
    ...
    foreach( $hits2 as $propValue => $occCnt ) {
      if( FALSE === ( $pos = utf8_strpos( $propValue, '@' )))
        continue;
...
function _addXMLchild( & $parent, $name, $type, $content, $params=array()) {
  ...
  $child = $parent->addChild( utf8_strtolower( $name ));
            /** fix attributes */
  ...
  if( !empty( $params )) {
    ...
    foreach( $params as $param => $parVal ) {
      $param = utf8_strtolower( $param );
      if( 'x-' == utf8_substr( $param, 0, 2  )) {
  ...
  switch( $type ) {
    ...
    case 'recur':
      foreach( $content as $rulelabel => $rulevalue ) {
        $rulelabel = utf8_strtolower( $rulelabel );
        switch( $rulelabel ) {
          case 'until':
    ...
    case 'uri':
      ...
      break;
    case 'utc-offset':
      if( in_array( utf8_substr( $content, 0, 1 ), array( '-', '+' ))) {
        $str     = utf8_substr( $content, 0, 1 );
        $content = utf8_substr( $content, 1 );
      }
      else
        $str     = '+';
      $str .= utf8_substr( $content, 0, 2 ).':'.utf8_substr( $content, 2, 2 );
      if( 4 < utf8_strlen( $content ))
        $str .= ':'.utf8_substr( $content, 4 );
      $v = $child->addChild( $type, $str );
      break;
    case 'unknown':
    default:
      ...
      elseif(( 'properties' == $calPart->getName()) && ( 0 < $calPart->count())) {
        foreach( $calPart->children() as $calProp ) { // calendar properties
         $propName = $calProp->getName();
          if(( 'calscale' != $propName ) && ( 'method' != $propName ) && ( 'x-' != utf8_substr( $propName,0,2 )))
            continue;
          $params = array();
...
function _getXMLParams( & $parameters ) {
  ...
  foreach( $parameters->children() as $parameter ) { // single parameter key
    $key   = utf8_strtoupper( $parameter->getName());
    $value = array();
...
//        case 'rrule':
      ...
      case 'byday':
        ...
        if( 2 == utf8_strlen( $byday ))
          $value[$valueType][] = array( 'DAY' => $byday );
        else {
          $day = utf8_substr( $byday, -2 );
          $key = utf8_substr( $byday, 0, ( utf8_strlen( $byday ) - 2 ));
...
function expandTimezoneDates($vtzc) {
    ...
    if(( '-' != utf8_substr( (string) $tzbefore['offsetSec'], 0, 1 )) && ( '+' != utf8_substr( (string) $tzbefore['offsetSec'], 0, 1 )))
    ...
    if(( '-' != utf8_substr( (string) $tzafter['offsetSec'], 0, 1 )) && ( '+' != utf8_substr( (string) $tzafter['offsetSec'], 0, 1 )))
